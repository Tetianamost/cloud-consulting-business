...
import { BarChart3, MessageSquare, FileText, Mail, Bot } from "lucide-react";
 ...
 */
const V0Sidebar: React.FC<V0SidebarProps> = React.memo(
  ({ currentPath, onNavigate, isMobile = false }) => {
    ...
    return (
      <div
        className={`
        ${isMobile ? "flex w-full" : "hidden lg:flex lg:w-64"} 
      ...
      >
        ...
        <nav
          className={`flex-1 space-y-1 ${isMobile ? "px-3 py-4" : "px-4 py-6"}`}
        >
          {navItems.map((item) => {
